...
for (i in 0:1000){if (i%%2==0) sum=sum+i}
print(sum)
...
for (i in 1:nrow(survey)){
  for(j in 2:ncol(survey)){
    #print(survey[i,j])
    if (survey[i,j]<1 | survey[i,j]>5){survey.clean[i,j]<- NA}
    if (is.integer(survey[i,j])==FALSE){survey[i,j]<-NA}
...
```{r}
invalid.answers=vector("numeric",length=6)
...
for (i in 1:nrow(survey.clean)){
 for(j in 2:ncol(survey.clean)){
    ...
    if (is.na(survey.clean[i,j])){survey.clean[i,ncol(survey.clean)]=survey.clean[i,ncol(survey.clean)]+1}
  }
}
survey.clean
```
